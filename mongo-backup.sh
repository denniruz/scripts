...
if [[ ${_nodetype} =~ "-prod" ]]
then
    _s3path="/backups/mongodb/prod/$(/bin/date +%Y)/$(/bin/date +%m)/$(/bin/date +%d)"
else
    _s3path="/backups/mongodb/nonprod/$(/bin/date +%Y)/$(/bin/date +%m)/$(/bin/date +%d)"
fi
...
#declare -a _db_list=( $(awk -F\" '!/local/ {print $2}' <(mongo localhost:27018 --eval "rs.slaveOk();printjson(db.getMongo().getDBNames())")) )
declare -a _db_list=( $(awk -F\" '!/local|test/ {print $2}' <(mongo localhost:27018 --eval "rs.slaveOk();printjson(db.getMongo().getDBNames().forEach(printjson))")) )
# Set some functions
usage() {
    cat <<EOF
    usage: $0 options
...
backup() {
    ...
    then
        _mongoopts=" --forceTableScan --ssl"
    else
        _mongoopts=" --forceTableScan"
    fi
...
archive() {
    ...
    do 
        echo ${_db_name}
        tar jcf ${_backup_dir}/${_nodetype}-${_hostname}-${_db_name}-$(date +%F).tar.bz2 ${_backup_dir}/${_db_name} >> ${_logfile} 2>&1
        if [[ ${_upload2aws} == "1" ]]
        then
            /opt/cronos/bin/s3Put.py -e -f ${_backup_dir}/${_nodetype}-${_hostname}-${_db_name}-$(date +%F).tar.bz2 -o ${_s3path}/${_nodetype}-${_hostname}-${_db_name}-$(date +%F_%H).tar.bz2 >> ${_logfile} 2>&1
            # Log backups to a local file for easy restore
            echo ${_s3path}/${_nodetype}-${_hostname}-${_db_name}.tar.bz2 >> ${_s3_logfile}
        fi
    # Clean up
    find /mnt/backups/mongodb -type f -mtime 2 -exec rm {} \;
    done
}
